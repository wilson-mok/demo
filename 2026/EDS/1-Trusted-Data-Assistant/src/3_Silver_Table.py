...
# business
df_business_bronze = spark.table(f"{table_namespace}.bronze_business")
...
df_business_silver.write.mode("overwrite").saveAsTable(f"{table_namespace}.silver_business")

# COMMAND ----------

# user
df_user_bronze = spark.table(f"{table_namespace}.bronze_user")

df_user_silver = df_user_bronze.select(
    "user_id",
    "name",
...
df_user_silver.write.mode("overwrite").saveAsTable(f"{table_namespace}.silver_user")

# COMMAND ----------

# review
df_review_bronze = spark.table(f"{table_namespace}.bronze_review")

df_review_silver = df_review_bronze.select(
    "review_id",
    "user_id",
    "business_id",
...
df_review_silver.write.mode("overwrite").saveAsTable(f"{table_namespace}.silver_review")

# COMMAND ----------

# tip
df_tip_bronze = spark.table(f"{table_namespace}.bronze_tip")

df_tip_silver = df_tip_bronze.select(
    "user_id",
    "business_id",
...
df_tip_silver.write.mode("overwrite").saveAsTable(f"{table_namespace}.silver_tip")