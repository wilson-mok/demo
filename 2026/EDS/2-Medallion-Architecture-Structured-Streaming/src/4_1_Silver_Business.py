...
transformed_stream = (bronze_stream
    ...
    # 1. City normalization (replace city column)
    .withColumn(
        "city",
    ...
    # 2. Rename stars and review_count for clarity
    .withColumnRenamed("stars", "lifetime_avg_stars")
    ...
    # Drop the original Bronze metadata columns (we've copied them with new names)
    .drop("_input_filename", "_input_file_modification_date")
)

print("Transformations applied:")
print("  ✓ City normalization applied")
...
print("  ✓ write_to_silver() - MERGE on key (business_id)")
...
def process_micro_batch(microBatchDF, batchId):
    ...
    # Deduplicate within micro-batch - keep latest record per business_id
    window_spec = Window.partitionBy("business_id").orderBy(col("_bronze_processing_date").desc())
...
query = (transformed_stream
    ...
    .outputMode("update")
    .queryName("stream-yelp_silver_business")
    .start()
)
...
# MAGIC   MIN(lifetime_review_count) AS min_review_count,
# MAGIC   MAX(lifetime_review_count) AS max_review_count
# MAGIC FROM demo_catalog.demo_yelp_academic.ss_silver_business;
...
# MAGIC %sql
# MAGIC
# MAGIC SELECT business_id, count(*)
# MAGIC FROM demo_catalog.demo_yelp_academic.ss_silver_business
